...
import { SquarePen, PawPrint, ShieldCheck, Calendar, ChevronDown, CheckCircle2, Image as ImageIcon, X, Lightbulb, RotateCcw, Lock } from "lucide-react";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { useCallback, useRef, useState } from "react";
import toast from "react-hot-toast";

export function EditButton({ list }) {

  const router = useRouter()

  const { vaccineStatus, species, petName, petBio, ownerEmail, location, imageUrl, healthStatus, gender, colorMarkings, breed, age, adoptionFee, _id } = list;
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.modifiedCount > 0) {
      ...
      toast.error("Failed update pet data");
    }
  };
            ...
            <Modal.Backdrop className="h-full " UNSTABLE_portalContainer={portalContainer}>
              <Modal.Container className="h-full max-h-full">
                <Modal.Dialog className="h-full max-h-full rounded-none w-[calc(100vw-100px)] max-w-none! mx-4  ">
                  ...
                  <Modal.Header>
                    <Modal.Heading>Edit your pet Details</Modal.Heading>
                  </Modal.Header>
                          ...
                          <div className="lg:col-span-2 space-y-8">
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-4">
                                <PawPrint className="w-4 h-4 text-[#c27d70] fill-current" />
